...
- **Social media corpora (v0.2).** A dedicated Social Media tab builds social corpora from the researcher's own exports (X archive, Instagram/Facebook Download Your Information, TikTok export, generic CSV/JSONL) entirely offline, or through official free-tier APIs (Mastodon, Reddit, YouTube) with researcher-supplied credentials. Text, emoji, hashtags, and engagement are analysed with the same statistical battery; attached images flow into the vision pipeline. There is no scraper in Lens and login-walled content is never requested; every import records a provenance attestation, and redaction/pseudonymisation are applied before storage.
- **Export & reporting**: xlsx/csv/tsv/xml/json per image and per set, CSV or XML export of every analysis outcome (battery results, social analytics, KWIC), plus an auto-drafted **Methods Section** naming the exact detector/OCR/embedding/vision-LM versions and formula versions used.
...
**One installer per platform — everything is inside.** The statistical/vision engine ships *within* every installer as a bundled sidecar; there is no second download and no Python to install. Since v0.2.1 the sidecar bundles the CPU-only torch/transformers stack, so object detection and CLIP scene/alignment analysis work out of the box; installers are correspondingly larger (hundreds of MB) and the first launch self-extracts more slowly. The Settings screen reports exactly which model stacks this installation carries.

| Platform | File | Covers |
|---|---|---|
| Windows 10/11 (64-bit) | [`CorpusMind.Lens_0.3.2_x64-setup.exe`](https://github.com/waleedmandour/CorpusMind-Lens/releases/download/v0.3.2/CorpusMind.Lens_0.3.2_x64-setup.exe) | Recommended Windows installer (per-user) |
| Windows 10/11 (64-bit) | [`CorpusMind.Lens_0.3.2_x64_en-US.msi`](https://github.com/waleedmandour/CorpusMind-Lens/releases/download/v0.3.2/CorpusMind.Lens_0.3.2_x64_en-US.msi) | Enterprise/GPO deployment |
| macOS 13+ (Intel **and** Apple Silicon) | [`CorpusMind.Lens_0.3.2_universal.dmg`](https://github.com/waleedmandour/CorpusMind-Lens/releases/download/v0.3.2/CorpusMind.Lens_0.3.2_universal.dmg) | One universal build for both architectures |
| Linux (Debian/Ubuntu) | [`CorpusMind.Lens_0.3.2_amd64.deb`](https://github.com/waleedmandour/CorpusMind-Lens/releases/download/v0.3.2/CorpusMind.Lens_0.3.2_amd64.deb) | deb-based distributions |
| Linux (most distributions) | [`CorpusMind.Lens_0.3.2_amd64.AppImage`](https://github.com/waleedmandour/CorpusMind-Lens/releases/download/v0.3.2/CorpusMind.Lens_0.3.2_amd64.AppImage) | Portable, no installation |
...
cd lens-engine
pip install -e ".[ocr]"
# Optional heavyweight AI stacks (object/scene detection, CLIP alignment):
# pip install -e ".[models]"  (CPU wheels: --extra-index-url https://download.pytorch.org/whl/cpu)
